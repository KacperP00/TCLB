...
CASE_DIR = sys.argv[2] if len(sys.argv) > 2 else "output/"
VTK_PREFIX = sys.argv[1] if len(sys.argv) > 1 else "02empty_cav_VTK_P00_"
PLOT_DIR = "wykresy/"
...
def temporal_transform(data_matrix, time_steps, omega, dt):
    # Całkowanie Fouriera w domenie czasu
    complex_amps = []
    t = time_steps
    t_f = t[-1] - t[0]
...
def residuals_upstream(params, x, eta_obs):
    # Model fali padającej i odbitej z optymalizacją k_complex
...
def residuals_downstream_fixed_k(params, x, eta_obs, k_complex):
    # Model fali transmitowanej (wymuszone k_complex)
    ReC, ImC = params
    C = ReC + 1j * ImC
...
def calc_stats(res, x_coords, eta_vals):
    # Obliczanie jakości dopasowania
